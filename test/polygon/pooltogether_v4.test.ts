    ...
    it("Returns the positions correctly for a USDC Prize Pool", async () => {
      ...
      for (let i = 0; i < prizePoolData.length; i++) {
        ...
        for (let j = 0; j < draws.length; j++) {
          ...
          console.log(
            draws[j].drawId,
            prizeDistributionsData[j].bitRangeSize,
            prizeDistributionsData[j].matchCardinality,
            prizeDistributionsData[j].numberOfPicks.toString(),
            formatUnits(drawsData.totalSupply[j], ticketData.decimals),
            prizeDistributionsData[j].prize.toString(),
          );
          // totalNumberOfPicks,
